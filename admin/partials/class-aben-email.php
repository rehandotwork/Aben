...
class Aben_Email {
    ...
    public function aben_email_template() {
        ...
        <title></title></head>
        <body>
        <div style="font-family:Open Sans,sans-serif;margin:0;padding:0;background: '.$this->body_bg.';color: #1f2430;">
        <div style="width:100%;max-width:500px;margin: auto;">
        <div style="padding:20px;">
        <p style="font-size:16px"><strong>'.$this->header_text.'</strong>
        ...
        <p style="font-size:16px;">'.$this->header_subtext.'</p></div>
        ...
        foreach ($this->posts_to_send as $post) {
            ...
            if ($this->show_view_post) {
                echo '<a href="' . $link . '"style="display:inline-block;padding:5px 20px;color:#fff;text-decoration:none;background-color:#0ead5d;border-radius:25px;height:fit-content">'.$this->view_post_text.'</a>';
            }
            echo '</div></div>';
        }
        ...
        if ($this->show_view_all) {
            echo '<a href="'.$this->archive_page_slug.'"style="display:inline-block;padding:15px 0px;background-color:#165d31;color:#ffffff;text-decoration:none;width: 100%;font-size:16px;">'.$this->view_all_posts_text;
            if ($this->show_number_view_all) {
                echo ' ('.$this->number_of_posts.')';
            }
            echo '</a>';
        }
        echo '</div></div></div>
        ...
        <p>'.$this->footer_text.'</p>';
        if ($this->show_unsubscribe) {
            echo '<p><a href="'.$this->unsubscribe_link.'" style="color:#808080;text-decoration:none">Unsubscribe</a></p>';
        }
